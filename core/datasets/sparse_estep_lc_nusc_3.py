...
class SparseEStepLCNusc3(dict):
    def __init__(self, root, voxel_size, version, verbose, **kwargs):
        ...
        super(SparseEStepLCNusc3, self).__init__({
            "train": _SparseEStepLCNusc3(nusc=self.nusc, voxel_size=voxel_size, split="train",
                                         image_crop_rate=im_cr, use_color=use_color, pseudo_label_dir=pseudo_label_dir),
            "val": _SparseEStepLCNusc3(nusc=self.nusc, voxel_size=voxel_size, split="val",
                                       image_crop_rate=im_cr, use_color=use_color, pseudo_label_dir=pseudo_label_dir)
        })


class _SparseEStepLCNusc3(data.Dataset):
    labels_mapping = {
        1: 0,
        5: 0,
        7: 0,
    ...
    def __init__(self, nusc, voxel_size, split="train", **kwargs):
        ...
        self.sparse_label_dir = os.path.join('./data/nuscenes/less_label_2', 'sparse_label')
        self.prop_label_dir = os.path.join('./data/nuscenes/less_label_2', 'prop_label')
        self.neg_label_dir = os.path.join('./data/nuscenes/less_label_2', 'neg_label')
    ...
    def __getitem__(self, index):
        ...
        if self.split == "test":
            ...
            labels_tensor = SparseTensor(labels_full, voxel)
