...
class NuScenesAssoP2PLCPLSparseLabel2(dict):
    def __init__(self, root, voxel_size, version, verbose, **kwargs):
        ...
        super(NuScenesAssoP2PLCPLSparseLabel2, self).__init__({
            "train": _NuScenesInternalAssoP2PLCPLSparseLabel2(nusc=self.nusc, voxel_size=voxel_size, split="train",
                                                              image_crop_rate=im_cr, superpixel=superpixel,
                                                              pseudo_label_dir=pseudo_label_dir,
                                                              use_augment=use_augment),
            "val": _NuScenesInternalAssoP2PLCPLSparseLabel2(nusc=self.nusc, voxel_size=voxel_size, split="val",
                                                            image_crop_rate=im_cr, superpixel=superpixel,
                                                            use_augment=False)
        })


class _NuScenesInternalAssoP2PLCPLSparseLabel2(data.Dataset):
    labels_mapping = {
        1: 0,
        5: 0,
        7: 0,
    ...
    def __init__(self, nusc, voxel_size, split="train", **kwargs):
        ...
        assert os.path.exists(self.superpixel_dir)
    ...
    def _fetch_asso_target(self, sp_labels, sem_labels, drop_rate=0.0):
    ...
    def __getitem__(self, index):
        ...
        if self.split == "test":
            ...
            if self.split == 'train':
                ...
                pseudo_label = np.full(shape=(other_mask.shape[0],), fill_value=self.ignored_labels, dtype=np.uint8)
                pseudo_label[other_mask] = pl
        ...
        if self.split == 'train':
            ...
            feed_dict['pseudo_label'] = SparseTensor(pseudo_label[inds], voxel_full)
