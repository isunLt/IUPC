...
__all__ = ['SPVCNN_SWIFTNET18']


class SPVCNN_SWIFTNET18(nn.Module):

    def __init__(self, **kwargs):
        super(SPVCNN_SWIFTNET18, self).__init__()
        ...
        try:
            build_image_decoder = configs['model']['build_image_decoder']
        except KeyError:
            print('cannot find configs[model][build_image_decoder], set to True by default')
            build_image_decoder = True
        imagenet_pretrain = configs['model']['imagenet_pretrain']
        self.pix_branch = SwiftNetRes18(num_feature=(128, 128, 128), pretrained_path=imagenet_pretrain, build_decoder=build_image_decoder)
        ...
        self.proj_channel = kwargs.get('proj_channel', cs[-1])
        try:
            self.run_image_decoder = configs['model']['run_image_decoder']
        except KeyError:
            print('cannot find configs[model][run_image_decoder], set to True by default')
            self.run_image_decoder = True
