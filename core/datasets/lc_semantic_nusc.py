...
class LCNuScenes(dict):
    def __init__(self, root, voxel_size, version, verbose, **kwargs):
        ...
        super(LCNuScenes, self).__init__({
            "train": _LCNuScenesInternal(nusc=self.nusc, voxel_size=voxel_size, split="train"),
            "val": _LCNuScenesInternal(nusc=self.nusc, voxel_size=voxel_size, split="val")
        })


class _LCNuScenesInternal(data.Dataset):
    labels_mapping = {
        1: 0,
        5: 0,
        7: 0,
